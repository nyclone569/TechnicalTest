...
{
	if (!CurrentLockedActor || UMyFunctionLibrary::NativeDoesActorHaveTag(CurrentLockedActor, MyGamePlayTags::Shared_Status_Dead) || UMyFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), MyGamePlayTags::Shared_Status_Dead))
	{
	...
	const bool bShouldOverrideRotation = !UMyFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), MyGamePlayTags::Player_Status_Rolling) && !UMyFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), MyGamePlayTags::Player_Status_Blocking);
...
void UHeroGameplayAbility_TargetLock::SwitchTarget(const FGameplayTag& InSwitchDirectionTag)
{
	...
	if (InSwitchDirectionTag == MyGamePlayTags::Player_Event_SwitchTarget_Left)
	{
...
AActor* UHeroGameplayAbility_TargetLock::GetNearestActorFromAvailableActors(const TArray<AActor*>& InAvailableActors)
{
...
void UHeroGameplayAbility_TargetLock::GetAvailableActorsAroundTarget(TArray<AActor*>& OutActorsOnLeft, TArray<AActor*>& OutActorsOnRight)
{
	...
	const FVector PlayerToCurrentNomalized = (CurrentLockedActor->GetActorLocation() - PlayerLocation).GetSafeNormal();
	...
	for (AActor* AvailableActor : AvailableActorsToLock)
	{
		...
		const FVector CrossResult = FVector::CrossProduct(PlayerToCurrentNomalized, PlayerToAvailableNormalized);
...
void UHeroGameplayAbility_TargetLock::DrawTargetLockWidget()
{
	...
	{
		checkf(TargetLockWidgetClass, TEXT("Forgot to assign a valid widget in Blueprint"));
...
void UHeroGameplayAbility_TargetLock::SetTargetLockWidgetPosition()
{
	...
	ScreenPosition.Y -= TargetLockWidgetSize.Y * 2.f;
